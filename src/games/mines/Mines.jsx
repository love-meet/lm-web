...
// Fit text into a single line within its container. If it overflows, append " ....".
function FitText({ text, className = '', suffix = '....', title }) {
  ...
  const recompute = React.useCallback(() => {
    ...
    // Reserve space for suffix and shrink until it fits
    let low = 0
  ...
  return (
    <span
      ...
      title={title ?? text}
    >
      <span aria-hidden style={{ position: 'absolute', visibility: 'hidden', whiteSpace: 'nowrap', pointerEvents: 'none' }} ref={measureRef} />
      {display}
    </span>
  )
}
...
export default function Mines() {
  ...
  // Prevent background scroll and bounce revealing underlying tabs while modal is open
  useEffect(() => {
        ...
        <div className="px-4 mt-4 mb-2">
          {gameStatus === 'waiting' ? (
            <div className="flex flex-col items-center text-center mb-4">
              <div className="text-pink-400 font-semibold mb-2 text-[15px]">Waiting for Partner...</div>
              <div className="flex items-center justify-between w-full max-w-xs">
                <div className="flex items-center gap-2 bg-slate-800/60 px-2.5 py-1.5 rounded-full border border-white/10 w-[120px] justify-center">
                  <span className="text-xs font-medium">Waiting...</span>
                </div>
                <Heart className="text-pink-400 mx-2" size={18} />
                <div className="flex items-center gap-2 bg-slate-800/60 px-2.5 py-1.5 rounded-full border border-white/10 w-[120px] justify-center min-w-0">
                  <img
                    src="/assets/default-profile.jpg"
                    alt="opponent"
                    className="h-6 w-6 rounded-full object-cover"
                  />
                  <FitText text={'Wisdom am'} className="text-xs font-medium max-w-full" />
                </div>
              </div>
            </div>
          ) : (
    ...
    />
        ...
        <div className="px-4 mt-4">
          <div className="bg-[#111827]/70 rounded-2xl border border-white/10 p-4 shadow-inner">
            {roundState === 'playing' ? (
              <>
                <p className="text-center text-sm opacity-80 mb-3">What is the name of this puzzle?</p>
              ...
              <div className="h-48 rounded-xl grid place-items-center bg-slate-800/60 border border-white/5">
                <span className="text-lg font-semibold text-gradient-accent">Loading new puzzle...</span>
              </div>
            )}
            <div className="mt-6 grid grid-cols-2 gap-3">
              <div className="text-center">
                <FitText text={'Benny Love'} className="text-sm font-semibold max-w-[140px] mx-auto" />
